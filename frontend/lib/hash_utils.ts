// Using NodeCanvas in server context, browser Canvas in client context
let Canvas: any = null;
if (typeof window === 'undefined') {
    // Import canvas only in Node.js environment
    try {
        const canvasModule = require('canvas');
        Canvas = canvasModule.Canvas;
    } catch (e) {
        console.warn('Canvas module not available in this environment');
        // Provide a mock Canvas implementation that returns consistent values
        Canvas = {
            createCanvas: () => ({
                getContext: () => ({
                    fillStyle: '',
...
export function generateRandomCanvasFingerprint(): string {
    ...
    const canvas = typeof document !== 'undefined' 
        ...
        : Canvas ? new Canvas(200, 200) : null;
