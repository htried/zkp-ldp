...
template ValidateInitialState(num_ips, attribution_history_size) {
    ...
    var len = num_ips + num_ips + 2 + 2 + attribution_history_size + 1 + 1;
    ...
    for (var i = 0; i < attribution_history_size; i++) {
        state_hash_inputs[2*num_ips + 4 + i] <== attribution_history[i];
    }
    state_hash_inputs[2*num_ips + 4 + attribution_history_size] <== users_prf_seed;
    state_hash_inputs[2*num_ips + 5 + attribution_history_size] <== state_counter;
...
template ValidateServerResponse() {
    ...
    response_hasher.inputs[4] <== new_fingerprint_nonce;
...
template CreateUpdatedState(num_ips, attribution_history_size) {
    ...
    // Update attribution history - shift all values and add new one at the beginning
    ...
    for (var i = 0; i < attribution_history_size; i++) {
        new_state_hash_inputs[2*num_ips + 4 + i] <== newState_attribution_history[i];
    }
    new_state_hash_inputs[2*num_ips + 4 + attribution_history_size] <== new_user_prf_seed;
    new_state_hash_inputs[2*num_ips + 5 + attribution_history_size] <== new_state_counter;
...
template AttemptStateUpdate(num_ips, attribution_history_size) {
    ...
    overall_chain_search_match.out === 1;
